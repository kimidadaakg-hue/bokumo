...
def area_from_address(address: str) -> str:
    ...
    注意:
    - 「北海道○○市」を greedy regex で拾うと「北海道○○市」全体が
      マッチして「北海道○○」になる事故あり（2026-05-06）。
      なので「北海道」を明示的に剥がしてから市名を抽出する。
    ...
    if "北海道" in address:
        # ① 「北海道○○市」（順方向、Place Details の標準形式）
        # 「北海道」を明示的に剥がしてから、最短マッチ +? で市名を取る
        # これで「北海道釧路市」→「釧路」、「北海道北見市」→「北見」になる
        m = re.search(r"北海道\s*([^\s\d、,\-_／/]+?市)", address)
        if m:
            return re.sub(r"市$", "", m.group(1))
        # ② 「○○市 北海道」（逆方向、英語混じり住所）
        m = re.search(r"([^\s\d、,\-_／/]+?市)\s+北海道", address)
        if m:
            return re.sub(r"市$", "", m.group(1))
        # ③ 「○○郡△△町/村」のセット（「本町」のような地名混入防止）
        m = re.search(r"郡\s*([^\s\d、,\-_／/]+[町村])", address)
        if m:

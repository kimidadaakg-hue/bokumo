...
### ⑪ 顔ブラー / プライバシー保護（2026-05-07 追加・絶対遵守）

Instagram / TikTok に投稿する写真には、第三者（来店客・店員・TV画面の人物等）の顔が
映り込むことがある。プライバシー保護のため **必ずブラー処理を通す**。

**永続ルール**:
- ✅ `scripts/instagram/02c_blur_faces.py` が `raw/*.jpg` の顔を OpenCV YuNet で検出して
  ガウシアンブラー（顔 bbox + 周囲 15% マージン）で隠す
- ✅ `run_daily.py` の STEPS で `02b_classify_photos.py` の後・`03_generate_caption.py`
  の前に必ず実行する（順序入れ替え禁止）
- ✅ raw/ に上書き、二重実行防止のため `raw/.blurred` マーカーを作成
- ✅ 検出失敗（取りこぼし）は許容する仕様（ユーザー指定）。完全防御ではない
- ✅ モデル: `assets/models/face_detection_yunet_2023mar.onnx`（OpenCV Zoo, 約 232KB）
- ✅ 検出パラメータは現状ベスト: `score_threshold=0.6, nms=0.3, top_k=50`

**禁止事項**:
- ❌ 顔ブラーステップを STEPS から外す / コメントアウトする
- ❌ raw/ にブラー前画像を残して、後段（04_render等）が誤って未処理画像を拾うこと
- ❌ score_threshold を 0.5 以下に下げる（誤検出で料理が潰れる）
- ❌ ブラー後の検出結果を「目視確認なし」で公開する大規模変更（パラメータ変えたら
   1日分は必ず目視チェック）

**新規パイプライン追加時のチェック**:
- 写真を新たに加工するスクリプトを書くなら、必ず `raw/.blurred` マーカーの存在を
  前提にすること（マーカーがなければエラー or skip で安全側に倒す）
